...
class InMemoryRepo(ChatRepository):
    ...
    def _get_next_seq(self, conversation_id: str) -> int:
        """Generate the next sequence number for a conversation."""
    ...
    async def get_conversation_history(
        ...
        with self._lock:
            ...
            return conversation_events[-limit:] if limit is not None else conversation_events
    ...
    async def compact_deltas(
        ...
        with self._lock:
            ...
            # Remove deltas in reverse order
...
class SQLiteRepo(ChatRepository):
    ...
    async def get_conversation_history(
        ...
        query = """
            ...
            AND (
                ...
                OR (type = 'system_update' AND json_extract(extra, '$.visible_to_model') = true)
            )
            ORDER BY seq
        """
        params: list[Any] = [conversation_id]
    ...
    async def list_conversations(self) -> list[str]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT DISTINCT conversation_id FROM chat_events") as cursor:
                rows = await cursor.fetchall()
